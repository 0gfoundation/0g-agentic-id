...
/// @title CanonicalIdentityRegistryMock
/// @notice Faithful behavioural replica of the official ERC-8004
///         `IdentityRegistryUpgradeable` v2.0.0 deployed on 0G Galileo testnet
///         at 0x8004a818bfb912233c491871b3d84c89a494bd9e.
///
///         The on-chain values were verified against the live proxy:
///           getVersion() == "2.0.0", name() == "AgentIdentity",
///           symbol() == "AGENT", EIP-712 domain "ERC8004IdentityRegistry"/"1",
///           agentIds start at 0 (ownerOf(0) is a live third-party agent).
///
/// @dev Only the initializer differs from the official source: the live contract
///      ships as a v2 upgrade (`reinitializer(2) onlyOwner`, no Ownable/ERC721
///      name re-init); this mock uses a plain `initializer` and seeds name /
///      symbol / domain / owner so it can be deployed standalone in tests. All
///      observable behaviour AgenticID's binding relies on — 0-based numbering,
///      `register()` seeding agentWallet = msg.sender, the official 4-arg
///      EIP-712 `setAgentWallet`, the reserved "agentWallet" key, and
///      agentWallet-cleared-on-transfer — is reproduced verbatim.
contract CanonicalIdentityRegistryMock is
    ...
    EIP712Upgradeable
{
    ...
    function getMetadata(uint256 agentId, string memory metadataKey) external view returns (bytes memory) {
        return _getIdentityRegistryStorage()._metadata[agentId][metadataKey];
    }
    ...
    function setMetadata(uint256 agentId, string memory metadataKey, bytes memory metadataValue) external {
        ...
        _getIdentityRegistryStorage()._metadata[agentId][metadataKey] = metadataValue;
    ...
    function getAgentWallet(uint256 agentId) external view returns (address) {
        bytes memory walletData = _getIdentityRegistryStorage()._metadata[agentId]["agentWallet"];
    ...
    function setAgentWallet(
        ...
        _getIdentityRegistryStorage()._metadata[agentId]["agentWallet"] = abi.encodePacked(newWallet);
    ...
    function unsetAgentWallet(uint256 agentId) external {
        ...
        _getIdentityRegistryStorage()._metadata[agentId]["agentWallet"] = "";
    ...
    function _update(address to, uint256 tokenId, address auth) internal override returns (address) {
        ...
        if (from != address(0) && to != address(0)) {
            _getIdentityRegistryStorage()._metadata[tokenId]["agentWallet"] = "";
