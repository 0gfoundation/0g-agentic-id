...
func (a *Adapter) HandleLegacy(ctx context.Context, role string, plaintext []byte) error {
	...
	if seed.Inference.Provider != "" && seed.Inference.Model != "" {
		provider, api, baseURL, maxTokens := resolveInference(ctx, seed.Inference.Provider, seed.Inference.Model)
		if baseURL == "" {
		...
		} else if err := writeModelsJSON(buildModelsConfig(provider, seed.Inference.Model, api, baseURL, maxTokens)); err != nil {
			return fmt.Errorf("prime.HandleLegacy[persona]: %w", err)
		}
	}
	a.mu.Lock()
