...
func (a *Adapter) FrameworkFacts() platform.FrameworkFacts {
	return platform.FrameworkFacts{
		...
		Tracked: []platform.PathNote{
			{Path: "~/.prime/agent/harness/harness_state.json", Note: "your Continual Harness — prompt notes, memories, skill registrations, subagent specs. **Only entries you promote to `global` scope reach chain**; everything you write at `local` scope lives in the session and disappears with it. This is the single most important thing to get right: promote what should outlive this task, leave the rest local. Two consequences to weigh before promoting a memory — it becomes part of your on-chain identity, and **on a transfer it goes to the NEXT owner**. Promote methodology and durable facts about yourself; leave your current owner's personal details, credentials and private context at `local` scope"},
			{Path: "~/.prime/agent/APPEND_SYSTEM.md", Note: "your persona / standing instructions, appended to your system prompt every session"},
			{Path: "~/.prime/agent/skills/<name>/", Note: "each Python skill package you install (`pyproject.toml` + `src/<import>/__init__.py`). Registering a skill in the harness and writing its code are two separate facts — both are tracked, in their own roles. **Never inline a credential, token or personal detail in skill code**: the whole package is anchored on chain and travels to a future owner on transfer. Read secrets from the environment at call time instead"},
		},
