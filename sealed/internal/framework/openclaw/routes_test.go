...
// openclaw exposes ONLY the chat route (/v1/). The control UI ("/") is
// deliberately not declared — it embeds a web terminal (ghostty /pty) +
// file browser, a shell/file backdoor if proxied. This guards against it
// being re-added.
func TestFrameworkRoutes(t *testing.T) {
	...
	// never signed (the proxy enforces this for all framework routes; the
	// declaration matches so it isn't misleading).
	...
	for _, r := range routes {
		if r.Kind == "dashboard" || r.Prefix == "/" {
			t.Error("control-UI/root route must NOT be declared — it exposes the terminal + file-browser backdoor")
		}
	}
}

// AuthResponse now returns only the credential — where/how to use it lives in
// FrameworkRoutes / /hello, not in the auth payload.
func TestAuthResponse_TokenOnly(t *testing.T) {
	a := &Adapter{authToken: "deadbeef"}
	...
	if _, present := m["dashboard_url"]; present {
		t.Error("dashboard_url must no longer be in the auth payload")
	}
	if len(m) != 1 {
