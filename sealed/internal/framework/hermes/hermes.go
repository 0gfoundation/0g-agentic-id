...
// FrameworkRoutes implements framework.RouteProvider. ONLY the chat API is
// exposed through the sealed proxy (/v1/, the OpenAI-compatible API server
// on :8642). The web dashboard (:9119) is deliberately NOT declared and NOT
// started: it embeds a terminal (/api/pty) + file browser, so behind the
// proxy it would give a token-bearing owner a shell + arbitrary file read
// inside the TEE — a backdoor around sealed isolation (same reasoning as
// openclaw's control UI). Owner interaction is the constrained chat API +
// SDK, which can't shell or read files.
...
// AuthResponse implements framework.Framework. Returns the single owner
// credential ({token}); how to present it per route is declared in
// FrameworkRoutes and surfaced via /hello. Same shape as openclaw — the
// SDK's authenticate() consumes only this token field.
//
...
func (a *Adapter) AuthResponse(ctx context.Context) (any, error) {
	...
	return map[string]any{"token": key}, nil
}

// Stop SIGTERMs the gateway process, waits up to gracefulTimeout, then
// SIGKILLs. Afterwards sweeps any leftover `hermes gateway run` process so
// a stale child can't hold :8642 against the next Start.
func (a *Adapter) Stop(ctx context.Context, gracefulTimeout time.Duration) error {
	...
	err := stopProc(ctx, cmd, gracefulTimeout)
	sweepOrphans()
	return err
}

// stopProc gracefully terminates one process: SIGTERM, wait up to graceful,
// then SIGKILL. Returns ctx.Err() if ctx cancels first. nil cmd is a no-op.
func stopProc(ctx context.Context, cmd *exec.Cmd, graceful time.Duration) error {
...
// sweepOrphans SIGKILLs any leftover hermes gateway process.
// pkill exit 1 = "no process matched", the happy path.
func sweepOrphans() {
	for _, pat := range []string{"hermes gateway run"} {
		out, err := exec.Command("pkill", "-9", "-f", pat).CombinedOutput()
		if err == nil {
			logger.Logf("hermes: swept orphan %q processes", pat)
			continue
		}
		if exitErr, ok := err.(*exec.ExitError); ok && exitErr.ExitCode() == 1 {
			continue // none matched
		}
		logger.Logf("hermes: pkill %q sweep failed: %v: %s", pat, err, strings.TrimSpace(string(out)))
	}
}
