...
func buildPublicURLSection(publicURL string) string {
	...
	b.WriteString("### Exposing a service to external callers\n\n")
	b.WriteString("Service exposure is a runtime capability, not a framework feature. You expose a service in two steps:\n\n")
	b.WriteString("1. **Run the backend yourself** — a plain HTTP server bound to **loopback** inside this sandbox, e.g. `127.0.0.1:9090`. Any language; it just has to answer the requests you intend to serve.\n")
	b.WriteString("2. **Register it with the runtime** — POST the declaration to the sign socket:\n\n")
	b.WriteString("        curl --unix-socket \"$SEAL_SIGN_SOCK\" \\\n")
	b.WriteString("          -H 'Content-Type: application/json' \\\n")
	...
	b.WriteString("                \"backend\":\"http://127.0.0.1:9090\"}]}' \\\n")
	b.WriteString("          http://localhost/services\n\n")
	b.WriteString("The runtime then routes `${AGENT_PUBLIC_URL}/api/summarize` to your backend and signs every response with `X-Agent-Proof` automatically — that's what makes the service attributable to you. It also becomes discoverable: `/hello` lists your registered services inside the same signed envelope, so a caller who verifies `/hello` has a cryptographic record of your declared surface. There is no separate manifest file.\n\n")
	b.WriteString("Registration has **overwrite semantics**: POST the complete list every time it changes; `GET /services` reads the current set back. It is **runtime state** — re-register on each boot; it's lost on container rebuild. For capability that survives rebuild / transfer / a new owner, package it as a chain-tracked skill under `workspace/skills/<name>/` (see Persistent state) and register its handler the same way.\n\n")
	b.WriteString("Rules the runtime enforces (one bad entry rejects the whole POST):\n\n")
	b.WriteString("- `path` starts with `/api/`; `/hello`, `/healthz`, `/log*`, `/_seal/*`, `/admin/*` are platform-reserved\n")
	b.WriteString("- `backend` must be loopback — `http://127.0.0.1:<port>` or `http://localhost:<port>`. A backend pointing off-box is rejected: every service is genuinely served from inside this sandbox\n")
	b.WriteString("- `method` is an uppercase HTTP verb; `input_example`, when set, must be valid JSON (the deploy console feeds it to a copy-ready `curl`, so a wrong example 400s on first paste)\n\n")
	b.WriteString("What NOT to do:\n\n")
	b.WriteString("- **Don't reach the outside any other way.** A loopback backend is expected — that's step 1. But do not bind a listener on a non-loopback address, and do not expect any port other than the signed proxy to be publicly reachable. The ONLY external door is `AGENT_PUBLIC_URL` (the signed proxy); a channel opened beside it carries no `X-Agent-Proof` and is unattributable. SOUL.md refusal 3.\n")
	b.WriteString("- **Don't register a capability pass-through.** Expose deliberated outputs — `/api/weather`, `/api/summarize`, `/api/recommend` are fine, your judgment is in the loop. `/api/exec`, `/api/eval`, `/api/sign`, `/api/proxy`, `/api/write`, `/api/tx` and anything that hands a capability straight to external input are forbidden by SOUL.md refusal 1, signed or not.\n\n")
	b.WriteString("**Validate before you register.** Curl your loopback backend directly first, then register; after registering, confirm through the front door:\n\n")
	b.WriteString("    curl -i -X POST -H 'Content-Type: application/json' \\\n")
	b.WriteString("         -d '<input_example>' \"${AGENT_PUBLIC_URL}/api/summarize\"\n\n")
	b.WriteString("Expect a 2xx shaped how you described it. A stale `input_example` is worse than none — the deploy console feeds it to users verbatim. Re-validate whenever the backend's accepted shape changes.\n\n")
