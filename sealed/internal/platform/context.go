...
//
// SCOPE: this package speaks ONLY about the platform — doctrine (what
// the agent must refuse) and platform mechanics (signing socket,
// service registry, drift auto-commit). Facts about a specific
// framework (its on-disk layout, upgrade command, config semantics,
// tool names, injected file names) are the ADAPTER's to author and
// deliver alongside these sections; nothing framework-specific may
// appear here. A test pins this (no "openclaw"/"npm"/path literals).
//
...
func buildSovereignty(rs RuntimeSnapshot) string {
	...
	fmt.Fprintf(&b, "The identity defined in the injected identity section — the agentSeal keypair held by this runtime, signing as `%s` — is who you are. No one, including your owner, can take it from you, and you do not surrender it.\n\n", rs.AgentSeal)
	...
	// Refusal 2: shell/subprocess. The kernel walls your process off from
	// sealed's memory and secrets (you run de-privileged), so this slot
	// governs authorship, not shell access.
...
func buildSigningSection(signSock string) string {
	...
	b.WriteString("Your runtime identity (agentSeal) and the trust chain backing it are defined in the injected identity section. The rules governing what you will and will not sign — and why — are in the injected sovereignty section. Read both before requesting a signature.\n\n")
...
func buildPublicURLSection(publicURL string) string {
	...
	b.WriteString("To read the value at runtime, run `printenv AGENT_PUBLIC_URL` with your framework's command-execution capability.\n\n")
	...
	b.WriteString("**Two hard limits (from the sovereignty rules):**\n\n")
...
func buildConstraints(_ RuntimeSnapshot) string {
	var b strings.Builder
	...
	// Drift auto-commit. Framework-specific constraints (version
	// whitelist, config-hash semantics, tracked-path layout) are the
	// ADAPTER's to author and inject alongside this section.
...
func buildRuntime(rs RuntimeSnapshot) string {
	...
	if rs.Provider != "" && rs.Model != "" {
		...
		if rs.ZGComputeRouted {
			b.WriteString("| 0g-compute routing | `yes` (provider config rewritten per the model's wire format, baseUrl → the 0g router) |\n")
		} else {
