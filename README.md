...
| `sealId` | bytes32 | A random 32-byte handle generated by attestor at mint; the HKDF input for deriving `agent_seal_priv`. `sealId → agentId` is one-to-one |
...
and AgentSeal derivation starts from that master key.
